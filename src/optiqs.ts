 ...
 * @param payload A function that will be used in the reducer.
 * @returns An action with the given function as the payload.
 */
export function updateState<S>(payload: OptiqsStateUpdateFn<S>): OptiqsAction<S> 

/**
 * Creates an action that will be used in the reducer to update state in the store.
 * @param payloads An array of functions that will be used in the reducer. They will be applied in the order they are declared.
 * @returns An action with the given functions as the payload.
 */
export function updateState<S>(payloads: OptiqsStateUpdateFn<S>[]): OptiqsAction<S> 
 ...
 * @param action The dispatched action. You can optionally provide a type here to strongly type this reducer to only accept specific actions.
 * @returns The updated state.
 */
export function reducer<S, A extends Action = AnyAction>(state: S, action: A): S {
  if (isOptiqsAction(action)) {
